...
export function createAdmin(ctx, services) {
  ...
  async function admin(body) {
    ...
    if (cmd === 'givegold') {
      ...
      rec.gs.gold = old + amt;
      const now = Date.now();
      if (rec.gs) rec.gs.__savedAt = now; // v9.14：GM 发放同样 bump 时间戳/版本，防玩家旧档随后覆盖
      const sv = players.bumpSv(user);
      players.markDirty(user);
      return { ok: true, gold: Math.floor(rec.gs.gold), sv, msg: user + ' gold: ' + old + ' -> ' + rec.gs.gold };
    }
    ...
    if (cmd === 'giveitem') {
      ...
      const now = Date.now();
      rec.gs.__savedAt = now; // v9.14：同上
      const sv = players.bumpSv(user);
      players.markDirty(user);
      return { ok: true, item, qty: Math.floor(rec.gs.cargo[item]), sv, msg: user + ' got ' + qty + ' x ' + item };
    }
