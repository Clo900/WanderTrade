...
(function (HL) {
  ...
  const CONFIG = {
    revision: 'cfg-2026-09-20-height1',
    ...
    terrain: {
       ...
       * ⚠ 实际只有 **water 的边界**由这张表生效（累积比例 = 水陆分界）；
       *   `plain` / `hill` 的划分由下面的 `hillThreshold` 在地貌噪声值上直接切分
       *   （见 hex-world 的「平地 / 丘陵」段）—— `hill` 这一档在这里只用于保持
       *   累积比例与历史一致（水陆分界点不变），它自己不再决定任何格子。
       * 注意：山脉与峡谷不在这一层，它们由下面的 relief 通道决定。
       */
      landformRatios: [
        { key: 'water', ratio: 0.18 },
        { key: 'plain', ratio: 0.50 },
        { key: 'hill', ratio: 0.32 }
      ],
      /**
       * 丘陵判定阈值（作用在**地貌噪声值**上，与 landformRatios 无关）：
       * 噪声 ≥ 该值 ⇒ `landform = 'hill'`（渲染时叠一层连绵波，见 height.hillWave）。
       * 旧版这个 0.54 硬编码在 hex-world 里，配置里的 hill 份额因此形同废纸。
       */
      hillThreshold: 0.54,
       ...
       * 等值线）。⚠ v2.8 阶段二起这条通道**只有一个消费者**（山格排名）：
       * 丘陵起伏已改为 `height.hillWave`（地貌噪声通道上的连续场），旧版
       * 「contrast 顺带抬高丘陵」这条副作用随之消失。
       */
      relief: {
        octaves: 4,
        gain: 0.5,
         ...
         *
         * ⚠ 仅地图边界格会跳过：它们是岛屿轮廓，不能被演示规则改写。块内普通
         *   水格会显式提升为山地，保证 4×5 演示块完整连通；渲染层仍以水位裁剪山壳。
    ...
    /* ---------------- 世界高度（v2.8 阶段二：逐地块档位 + 丘陵连绵波）----------------
     * 逻辑层的高度是**离散的**：每个地块按类型落在一个「档位」上（见 `tiers`）。
     * 渲染层的高度是**连续的**：档位之上再叠一层圆润连绵的波（`hillWave`），
     * 河带走廊则被钉在局部河面上（见 `river.corridor`）。
     *
     * ⚠ 山格（terrain = 'ridge'）**不抬地表基座**：山体仍是独立网格层，山脚落在
     *   0 平面；山格的「档位」只用于**水文**（河流在山体里的河面取丘陵档），
     *   靠河槽下切与山体侵蚀把山壳切到水下，河因此嵌在山壳里而不是浮在山脚之上。
     */
    height: {
      /**
       * 旧「视觉最大高度」（× hexSize）。保留兼容：地表高度**不再**由它驱动，
       * 它只作为状态层的路面高度上限与 `world.maxRise` 的基准。⚠ 山体雪线曾按它
       * 取值，导致「53 座峰零雪顶」—— 雪线现在按每片自身峰高取，不要往这个值上挂
       * 新的高度判据。
       */
      visualPeak: 1.9,
      /**
       * 档位表（× hexSize）：**逻辑层的离散高度**，地块类型即高程。
       *   plain  平原 / 农田 / 森林 / 花田 —— 与海面齐平的基准平面
       *   hill   丘陵 —— 抬高一点点（**整体观感仍是平的**，只有高度感）
       *   city   城市广场 —— 保持基准平面
       * ⚠ 海面档位不在这里：它只有一个来源 `water.level`（`HeightField.tiers` 读它）。
       */
      tiers: { plain: 0, hill: 0.10, city: 0 },
      /**
       * 山格的水文档位（键名引用 `tiers`，不另写数值 —— 避免同一个高度有两处来源）。
       * 决策：河源落在山地时，河面取**丘陵档**；山格地表基座仍是 0（不抬）。
       */
      ridgeHydroTier: 'hill',
      /**
       * 丘陵的「圆润连绵的波」。
       * 与旧版的逐格 dome（每格一个圆包、格边归零）本质不同：这是一个**世界坐标上
       * 连续**的低频场，天然跨格连绵、共享角点自动一致，因此不会在格边留下台阶。
       * ⚠ 这里**不写幅度**：波形幅度由 `HeightField.hillWave` 从 `tiers.hill` 取
       *   （均值 = 丘陵档，峰值 = 2 × 丘陵档）。旧版这里还有一个 `meanAmp`，
       *   与 `tiers.hill` 是同一个数的两处来源 —— 改一处不会动另一处。
       *   scale    波的尺度（× hexSize）：一张「波长 ≈ scale 格」的连绵起伏
       *   band     过渡带半宽（**地貌噪声值**，≈1 格）：把「丘陵 / 平原」的硬阈值
       *            换成一条平滑带，于是丘陵边缘的平原会被带起一点点，再往外严格为 0。
       */
      hillWave: { scale: 5.0, band: 0.09 },
      /** 由地图中心向外的衰减，制造岛屿轮廓（作用于排名，不是阈值） */
      islandFalloff: 0.30,
      /**
       * 格内细分（中环）。
       ...
       * 只在**非丘陵**格上全额生效 —— 丘陵格已经有连绵波，再叠一层逐格噪声会
       * 变成「噪上加噪」（见 terrain-layer 的归一化）。
       */
      innerRelief: 0.030
    },
    ...
    palette: {
      ...
      /**
       * 各水体的浅水 / 深水色。**这是水面颜色的唯一来源**（v2.7）：
       * 旧版 `render/water-depth.js` 另存了一份 uDepthShallow / uDepthDeep，
       * 与表现层各混一遍 ⇒ 同一份 palette 被读两次、水色被往「淡」里拉两次。
       * 现在只有表现层读它，深度只负责透明度和深浅比例。
       *
       * 取值按《文明 6》参考图重新定：浅水是偏青的亮色、深水是饱和的蓝绿，
       * 两档亮度差 ≈ 0.3（旧版只有 0.15，全图看起来是一块淡淡的蓝白）。
       */
      water: {
        shallow: 0x9adfe0,
        deep: 0x2e7f9d,
        foam: 0xecf6f7,
        sea: { shallow: 0x9adfe0, deep: 0x2e7f9d },
        river: { shallow: 0x86cfd6, deep: 0x2a7590 },
        spring: { shallow: 0xa8e4e2, deep: 0x3f8fa0 }
      },

      /**
       * 河流地表配色（湿岸、河滩与冲积带），不再定义独立水面色。
       * bedTint 是地表在河线附近的混色目标：河床/滩涂的湿泥与砾石色，
       * 与水面一起才读得出「这是一条河」，只靠水带会像贴了一条蓝纸。
       * alluvial 是**河口冲积平原**的色带目标（v2.7）：由 `rivers.alluvial()`
       * 这种世界位置连续函数给出强度，与 floodplain 同一种做法。
       */
      river: {
        bedTint: 0x9f916f,     // 河滩混色（砾石/湿泥）
        alluvial: 0xd8c49a     // 河口冲积（细砂/淤泥）
      },
    ...
    water: {
       ...
       *   见 §15.24。
       */
      depthFade: 0.09,        // 默认 × hexSize：到这个水深就算「深水」（0.09 × 22 ≈ 1.98 单位）
      alphaMin: 0.40,         // 默认浅滩处水面的不透明度（透出水下地表）
      depthAlphaMin: 0.40,    // 兼容旧配置；新代码优先读取 alphaMin
      depthTint: 0.58,        // 保留：旧版「深水压深」的系数（新表现层不再读它）
      depthResolution: 0.5,   // 深度预通道的分辨率比例（0.5 = 半分辨率）

      /**
       * 「大片连续纯水」的面积下限（格）。**入海口的唯一判据**。
       *
       * ⚠ 为什么需要它（v2.7）：`distToLand` 只说「离岸多远」，1 格的水洼和 140 格
       *   的主海在它看来都是水。实测默认地图 10 个连通域：主海 140 格，其余 9 个
       *   只有 1~5 格 —— 而 6 条河里只有 2 条真的接进主海，另 2 条分别停在 4 格和
       *   1 格的水洼里，`reachesSea` 却报 true（它只检查「顶点是否贴水格」）。
       *   阈值取 12：5 → 12 → 140 双向都有余量，未来出现中型湖泊不会被误判。
       */
      seaMinBodyTiles: 12,
      ...
      /**
       * 手绘卡通水体的共享表现参数。**全部以 × hexSize 计**（与其它水体参数一致）。
       *
       * ⚠ 单位统一是 v2.7 修的一个真 bug：旧版 `waveAmplitude` 是唯一一个**不乘
       *   hexSize** 的水体参数，0.045 直接当世界单位用 ⇒ 振幅只有格距的 0.2%。
       *   实测把振幅 ×20 后画面变化的水像素从 0.97% 跳到 59.37%，即比「能看见」
       *   低了约 20 倍 —— 这就是「看不到波浪」的主因。
       *
       * ⚠ 波浪**只做在片元**（解析法线 + 色带），不做顶点位移：水面与陆地共面
       *   （`water.level = 0`，陆地也在 0），任何向上的顶点位移都会把水面顶到陆地
       *   之上；而且水面每格只有 13 个顶点（顶点间距 ≈ 11 单位），波长比采样间距
       *   还短，属于严重欠采样。片元做法同时解决这两件事，且与分辨率无关。
       */
      shared: {
        waveAmplitude: 0.07,    // 波形斜率尺度 ⇒ 0.07 × 22 ≈ 1.54。**同时**驱动片元法线与海面顶点位移（同一组正弦、同一个幅度）—— 剪影与明暗因此天然一致
        waveLength: 2.6,        // 主波长 ⇒ 2.6 × 22 ≈ 57 单位（约 2.6 格）
        waveSpeed: 0.55,        // 相位速度（弧度/秒）
        waveCross: 0.63,        // 第二组交叉波的频率比
        waveDetail: 0.15,       // 第三组细节波（高频、低幅）的权重
        bandSteps: 3,           // 手绘色带档数（cel shading 的档位）
        bandSoftness: 0.14,     // 色带交界的软度（0 = 硬边，1 = 平滑）
        specular: 0.34,         // 阶梯高光强度
        highlightSteps: 3,      // 高光档数
        absorption: 0.30,       // 波谷压深量（波谷更暗、波峰更亮）
        aoStrength: 0.16,       // 波浪凹陷处的 AO 强度
        edgeDarken: 0.20,       // 水带边缘压深（河/湖的岸线收边）
        foamNoise: 0.55,        // 泡沫边缘的噪声破边强度
        shorelineMotion: 0.50,  // 泡沫沿水线的缓慢脉动强度
        foamColor: 0xecf6f7
      },
      /**
       * 统一水面的几何参数（v2.8）。
       * ------------------------------------------------------------
       * 海 / 河 / 泉 现在合并成**一份几何 + 一份材质**（render/water-surface.js），
       * 所以「每个水型一套 renderBias」被整体删除 —— 它们曾把同一个逻辑水位拆成
       * **0 / +0.132 / +0.264** 三个高度（实测），河口因此出现 0.264 的台阶，
       * 河面像一块浮板压在海面上，末端断面就是「方头」。现在三者严格同高。
       *
       * `wave.subdiv`：开阔海面的**网格加密档**。顶点位移（让波浪在剪影上看得见）
       *   需要采样密度：水面每格原本只有 13 个顶点（1 圈中环），顶点间距 ≈ 11 单位，
       *   而主波长 57 单位 —— 5 倍采样做不出起伏轮廓。加密到 2 圈中环即可。
       *   河道与湖泊不加密、也不位移（位移会让水爬上岸）。
       */
      wave: { subdiv: 2 },
      /**
       * 河道末端收尖（按采样点）：末尾 `count` 个采样点的半宽线性收到 `minScale`。
       * 水面带原来是「到岸即断」的平头截面 —— 用户看到的「方头结尾」正是它。
       * 收尖只发生在**渲染预处理**里（改的是渲染用半宽副本），逻辑半宽、河床下切、
       * 切槽宽度与避让距离都不受影响。
       */
      mouthTaper: { count: 3, minScale: 0.35 },
      /**
       * 河口段的收尾（只影响表现）：
       *   · mouthFade  河口处额外乘上的透明度（0 = 不淡出，1 = 全透明）—— 让河水
       *                **融进**海面，而不是把海面切断成一条硬边；
       *   · mouthFoam  河口泡沫强度（只在 `aMouth > 0` 的顶点上生效，海 / 湖恒为 0）。
       */
      mouthFade: 0.55,
      mouthFoam: 0.55,
      /** 海：最宽最深，泡沫最窄、波纹最慢；三类水里只有它做顶点位移 */
      sea: {
        depthFade: 0.09, alphaMin: 0.40, waveSpeedMul: 0.70, absorptionMul: 1.00,
        foamWidth: 0.030, flowStrength: 0.20, mapStrength: 0.40, waveAmp: 1
      },
      /**
       * 河：浅而清、流动最快、泡沫带宽（河道边缘水深约 0.79 单位，见 channel.depth）。
       *   · waveAmp = 0      不做顶点位移（河面必须贴住切槽，位移会爬上岸）
       *   · mouthBlend       从中心线这个进度之后开始混向海色（河口段的起点）
       *   · mapStrength 最大 水纹在河面上最明显（顺流读得出来）
       * ⚠ 旧配置里还有一个 `foamMul`，但从 v2.7 起材质只读 `foamWidth`（泡沫带宽），
       *   `foamMul` 从来没被任何代码读过 —— 已删除，避免「改了不生效」的假参数。
       */
      river: {
        depthFade: 0.075, alphaMin: 0.48, waveSpeedMul: 1.60, absorptionMul: 0.75,
        foamWidth: 0.055, flowStrength: 1.00, mapStrength: 0.55,
        mouthBlend: 0.80, waveAmp: 0
      },
      /** 泉 / 小湖：最清亮，波纹最轻，同样不做顶点位移 */
      spring: {
        depthFade: 0.055, alphaMin: 0.34, waveSpeedMul: 0.90, absorptionMul: 0.60,
        foamWidth: 0.040, flowStrength: 0.65, mapStrength: 0.40, waveAmp: 0
      },

      /**
       * 可选高度图：资源失败、未配置或未启用时保持 sum-of-sines 回退。
       * `scale` 与 `bias` 同样以 × hexSize 计；只调制波浪振幅，不改水位、地块分类、
       * 河流寻路或河床高度。
       */
      heightmap: {
        enabled: false,
     ...
     * 整图一个水位，不跟随格内起伏爬坡。
     *
     * 宽度只有一个来源 `width`（**主河**统一半宽 × hexSize）：干流整条同宽，不随
     * 长度收放（参考文明 6：干流河宽基本恒定）。**支流**另有一套首尾半宽
     * （`tributary.widthSource → widthMouth`），沿中心线连续渐宽 —— 主河与支流
     * 共用同一份 `sample.halfW`，它同时服务河面、河床、侵蚀、湿岸与避让。
     * ⚠ 旧注释写「支流与干流同宽、不做锥形收放」，与实现（widthAt 渐宽）矛盾，v2.7 改正。
     *
     * 岸色带与避让全部是「水带半宽 × 倍数」：
    ...
    river: {
      enabled: true,
      maxRivers: 5,           // 最多几条河（从离「大片连续纯水」最远的山地/丘陵起点选）
      sourceSpacing: 5,       // 河源之间的最小格距
      maxSteps: 90,           // 单条河最多走多少条棱
      minLength: 5,           // 少于此长度的河丢弃（条棱数）

      /**
       * 轨迹走不到「大片连续纯水」（死路 / 被城市挡住 / 势能场尽头）时怎么办。
       *   `'drop'`  丢弃这条轨迹 —— **默认**。保证输出集里每条主河都真的入海。
       *             实测默认地图会因此丢掉 1 条（停在 1 格水洼里），主河 5 → 4 条。
       *   `'join'`  先尝试并入最近的另一条河（变成支流），只有实在接不上才丢弃。
       *             保河数，但会多出几条「不自己入海」的干流。
       * 注意这条开关**不影响**已汇流的轨迹：`joined` 的河本来就不需要自己的入海口。
       */
      onDeadEnd: 'drop',
      width: 0.16,            // 主河半宽（× hexSize）→ 水带宽 7 单位 ≈ 1/3 格
      ...
      /**
       * 河流沿程下降（v2.8 阶段二）。
       * 河面高度 = 所经地块的**档位**（地块类型即高程），因此从山源到海口会
       * 一级级落下来。两条参数：
       *   stepSlope  档间斜坡的**沿河长度**（格）：落差在一个短斜坡内完成，
       *              而不是垂直台阶（垂直台阶在地表网格上无法承载）。
       *   corridor   河带走廊的平坦带宽度（格，**在槽宽之外**）：走廊内的地表被
       *              钉在「该处河面 − 槽深」，于是水面永远被不低于它的地面围住
       *              （否则丘陵的波谷会让水带悬空）。0 = 关掉走廊（回到旧行为）。
       */
      stepSlope: 1.0,
      corridor: { band: 0.6 },
      /**
       * 河口延伸段（v2.8）：**寻路的收尾判据写在顶点上**，而顶点是水陆交界的角点 ——
       * 于是河面在海岸线上就断了，末端截面一半压陆、一半压水，外侧再没有任何几何
       * （实测末端外侧半格处的最近河面顶点距离 = 5.7~11 单位 = 空）。这就是「方头」。
       *
       * 这里让中心线**继续插进水格内部**，直到「离最近陆地格 ≥ minDistToLand 格」
       * 或走满 `maxSteps` 格。延伸段的采样点同样是逻辑数据（参与河道索引），
       * 因此切槽与河床会跟着延伸 —— 河口因此是真的开进海里，而不是停在岸线上。
       */
      mouthRun: { maxSteps: 2, minDistToLand: 1.0 },
      /**
      ...
      /**
       * 河源水体（泉眼 / 小湖）：在河源所在的格内刻一个**碗**，水面覆盖上去，
       * 于是河源有了源头，而不是一条和别处同宽的水带凭空开始。水位取
      ...
      tributary: {
        ...
        maxSteps: 20,
        minLength: 2,
        widthSource: 0.075,  // 支流河源半宽（× hexSize）
        widthMouth: 0.11      // 支流汇入口半宽（× hexSize）；沿中心线连续渐宽
      },

      /**
       * 河口三角洲 + 冲积平原（v2.7）。
       *
       * 分流只进**表现层**：从干流末端的 `forkT` 处向前分叉出 `branches` 条短水带，
       * 各自的中心线由 `river-builder` 生成（保证与干流同点、宽度递减），
       * 渲染层用 Ribbon 铺窄带。逻辑水位、寻路与河床都不受影响。
       *
       * 冲积平原按你的选择**只做表现色带**：河口周围 `alluvialRadius` 范围内的
       * 地表向 `alluvialColor` 靠拢（与现有 `floodplain()` 同一种「世界位置连续函数」
       * 的做法），不新增地块类别、不改地形高度与寻路。
       */
      delta: {
        enabled: true,
        branches: 3,          // 分流条数（2~3 条；1 条等于不分叉）
        forkT: 0.86,          // 分叉点位置：干流中心线的进度（0 源头 → 1 河口）
        length: 1.9,          // 分流长度（× hexSize）
        spread: 34,           // 分流总张角的一半（度）
        widthScale: 0.55,     // 分流半宽（× 该处干流半宽）
        bend: 0.35,           // 分流外弯量（× 分流长度）：越大越像散开的三角洲
        alluvialRadius: 1.35, // 冲积带半径（× 主河半宽）
        alluvialStrength: 0.42 // 冲积色带强度（颜色见 palette.river.alluvial）
      }
    },
