...
> **归档注记（v9.10.5）**：首批 5 文件 + v9.0 第二批（state/event-bus/runtime + 样式抽离）均已落地，后续又拆出 economy/map/gameplay 等更多模块（见 `Docs/地图模块实现索引.md`、`Docs/交易系统与物价引擎实现索引.md`）。本文作为**历史迁移记录**保留，不再跟踪现行模块边界。
