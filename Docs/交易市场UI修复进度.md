...
>
> **后续变更（v9.10.5 清理）**：单笔"立即买卖"入口 `doAction`/`doSell` 已随死代码清理删除（市场 UI 全部经中转面板批量 `draftFromSlider → confirmDraft → execBuyDraft/execSellDraft`）；本文为历史进度记录，现行实现以 `Docs/交易系统与物价引擎实现索引.md` 为准。
